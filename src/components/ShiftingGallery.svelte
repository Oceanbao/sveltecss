<script lang="ts">
	...
	onMount(() => {
		const box = document.getElementById('shiftinggallery-box') as HTMLElement;
		...
		const handlerOnMouseMove = (e: MouseEvent) => {
			...
			const xDecimal = mouseX / box.offsetWidth;
			const yDecimal = mouseY / box.offsetHeight;

			const maxX = gallery.offsetWidth - box.offsetWidth;
			const maxY = gallery.offsetHeight - box.offsetHeight;
		...
		box.addEventListener('mousemove', handlerOnMouseMove);

		return () => {
			box.removeEventListener('mousemove', handlerOnMouseMove);
		};
	});
</script>

<section id="shiftinggallery-box">
	<div id="gallery">
		<div class="tile">
			<img

<script lang="ts">
	...
	const handleLoveClick = () => {
		const groups = document.getElementsByClassName('card-group') as HTMLCollectionOf<HTMLElement>;

		const nextIndex = activeIndex + 1 <= groups.length - 1 ? activeIndex + 1 : 0;

		const currentGroup = document.querySelector(`[data-index="${activeIndex}"]`) as HTMLElement,
			nextGroup = document.querySelector(`[data-index="${nextIndex}"]`) as HTMLElement;
...
<style>
	...
	section :global(.card-group[data-status='after']) {
		transform: translateX(50%) scale(0);
	}

	section :global(.card-group[data-status='before']) {
		transform: translateX(-50%) scale(0);
	}

	section :global(.card-group[data-status='becoming-active-from-after']) {
		transform: translateX(50%) scale(0);
		transition: none;
	}

	section :global(.card-group[data-status='becoming-active-from-before']) {
		transform: translateX(-50%) scale(0);
		transition: none;
	}
